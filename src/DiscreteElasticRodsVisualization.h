...
public:
    ...
    void initVisualization(DiscreteElasticRods& discrete_elastic_rods, std::string name)
    {
        for (int i = 0; i<discrete_elastic_rods.nv; i++) {
            vis_nodes.emplace_back(discrete_elastic_rods.x(3*i), discrete_elastic_rods.x(3*i+1), discrete_elastic_rods.x(3*i+2));
        }
    ...
    {
        for (int i = 0; i<discrete_elastic_rods.nv; i++) {
            Eigen::Vector3d node(discrete_elastic_rods.x(3*i), discrete_elastic_rods.x(3*i+1), discrete_elastic_rods.x(3*i+2));
            vis_nodes[i] = node;
        }
